...
第 9 行 全 8 格：接着上一行，从正下方继续向左转，经过左下、左边、左上，转回接近正上方`,
  ...
  'settings.general.storagePortableDescription': "把数据保存到桌面端执行文件旁的 cc-haha-data 文件夹，移动整个安装目录到其他 Windows PC 后无需重新配置。",
  'settings.general.storageCustomDirPlaceholder': "选择一个用于保存 cc-haha 数据的文件夹",
  'settings.general.storageCustomDirLabel': "自定义数据目录",
  'settings.general.storageCustomDescription': "把桌面端数据写入你明确选择的文件夹。必须使用绝对路径，而且不能放在应用安装目录内。",
  'settings.general.storageCustomTitle': "使用自定义目录",
  ...
  'settings.general.storageApplyCustom': "使用此文件夹并重启",
  ...
  'settings.general.storageSwitchPortableBody': "切换后，桌面端会把数据存到执行文件旁的 cc-haha-data 文件夹。移动安装目录时一并移动该数据文件夹，即可在其他 Windows PC 上保持配置。",
  'settings.general.storageSwitchCustomBody': "切换后，桌面端会在这个自定义目录中读写会话、设置、Skills、MCP、插件、任务和缓存。",
  'settings.general.storageSwitchDefaultBody': "切回系统目录后，桌面端会重新使用 ~/.claude。自定义或便携目录中的数据不会被删除，也不会自动迁回。",
  ...
  'update.checksEnabledDescription': "允许自动和手动检测、下载应用更新。关闭后不会发起更新器网络请求。",
  'update.checksEnabled': "检测更新",
